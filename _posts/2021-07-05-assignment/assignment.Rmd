...
```{r}
carassign_final$company <- "GASTech"

# transform it to a edge list!
edges_level1_2 <- carassign_final %>% 
  select(company, CurrentEmploymentType) %>% 
  unique %>% 
  rename(from=company, to=CurrentEmploymentType)
edges_level2_3 <- carassign_final %>% 
  select(CurrentEmploymentType, CurrentEmploymentTitle) %>% 
  unique %>% rename(from=CurrentEmploymentType, to=CurrentEmploymentTitle)
edge_list=rbind(edges_level1_2, edges_level2_3)
 
# Now we can plot that
p6 <- graph_from_data_frame(edge_list)
ggraph(p6, layout = 'dendrogram', circular = FALSE) + 
  geom_edge_diagonal() +
  geom_node_point() +
  theme_void()
```
